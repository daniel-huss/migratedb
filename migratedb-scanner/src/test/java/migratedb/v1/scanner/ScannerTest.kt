...
import migratedb.v1.scanner.testing.Dsl
import migratedb.v1.scanner.testing.FsConfigurations
import migratedb.v1.scanner.testing.Kind.*
import migratedb.v1.scanner.testing.Mod.*
import migratedb.v1.testing.util.io.resolve
...
internal class ScannerTest {
    ...
    fun `Empty result on empty class path`(fsConfig: Configuration) = withDsl(fsConfig) {
        val result = Scanner().scan(migratedb.v1.scanner.Scanner.Config(emptySet(), emptySet()))
    ...
    fun `Empty result if no path is included`(fsConfig: Configuration) = withDsl(fsConfig) {
        ...
        val result = Scanner().scan(
            migratedb.v1.scanner.Scanner.Config(
    ...
    fun `Supports multi-release jar`(fsConfig: Configuration) = withDsl(fsConfig) {
        ...
        // when
        val result = Scanner().scan(migratedb.v1.scanner.Scanner.Config(setOf(jarFile), setOf("foo")))
    ...
    fun `Callback onUnprocessablePath works`(fsConfig: Configuration) = withDsl(fsConfig) {
        ...
        Scanner { callbackCount.incrementAndGet() }
            .scan(migratedb.v1.scanner.Scanner.Config(setOf(processablePath, unprocessablePath), setOf("foo")))
    ...
    fun `Works with ClassPathLocation`(fsConfig: Configuration) = withDsl(fsConfig) {
        ...
        // when
        Scanner().scan(migratedb.v1.scanner.Scanner.Config(setOf(classpathDir, jarFile), setOf("foo", "bar"))).also {
        ...
        actual.resourceProvider().getResources("", listOf(".sql")).map { it.name }
