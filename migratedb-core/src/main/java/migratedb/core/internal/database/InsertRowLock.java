...
public class InsertRowLock {
    ...
    /**
     * A random string used as an ID for this instance of MigrateDb.
     */
    ...
    public void doLock(String insertStatementTemplate, String updateLockStatement, String deleteExpiredLockStatement,
                       String booleanTrue) throws SQLException {
        int retryCount = 0;
        while (true) {
            try {
                ...
                if (retryCount < 50) {
                    retryCount++;
                    LOG.debug("Waiting for lock on MigrateDb schema history table");
                } else {
                    LOG.error(
                        "Waiting for lock on MigrateDb schema history table. Application may be deadlocked. Lock row " +
    ...
    private ScheduledFuture<?> startLockWatchingThread(String updateLockStatement) {
        Runnable lockUpdatingTask = () -> {
            LOG.debug("Updating lock in MigrateDb schema history table");
