...
public class PostgreSQLSchema extends Schema<PostgreSQLDatabase, PostgreSQLTable> {
     ...
     *
     * @param recreate Flag indicating whether the types should be recreated. Necessary for type-function chicken and
     *                 egg problem.
     *
     * @return The drop statements.
     *
    ...
    private List<String> generateDropStatementsForBaseTypes(boolean recreate) throws SQLException {
        List<Map<String, String>> rows =
            jdbcTemplate.queryForList(
                ...
                +
                "where (t.typrelid = 0 OR (SELECT c.relkind = 'c' FROM pg_catalog.pg_class c WHERE c.oid = t" +
                ".typrelid)) "
                +
    ...
    private List<String> generateDropStatementsForRoutines() throws SQLException {
        ...
        List<Map<String, String>> rows =
            jdbcTemplate.queryForList(
                ...
                "SELECT proname, oidvectortypes(proargtypes) AS args, " + isAggregate + " as agg, " + isProcedure +
                " as proc "
    ...
    private List<String> generateDropStatementsForEnums() throws SQLException {
        List<String> enumNames =
            jdbcTemplate.queryForStringList(
                "SELECT t.typname FROM pg_catalog.pg_type t INNER JOIN pg_catalog.pg_namespace n ON n.oid = t" +
                ".typnamespace WHERE n.nspname = ? and t.typtype = 'e'",
                name);
