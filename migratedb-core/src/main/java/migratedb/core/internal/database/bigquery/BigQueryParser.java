...
public class BigQueryParser extends BaseParser {
    ...
    private void handleAmbiguityStringLiteral(PeekingReader reader, char singleQuote, String tripleQuote)
    throws IOException {
    ...
    protected void adjustBlockDepth(ParserContext context, List<Token> tokens, Token keyword, PeekingReader reader) {
        ...
        if (lastTokenIs(tokens, parensDepth, "END") &&
            !"IF".equalsIgnoreCase(keywordText) && !"WHILE".equalsIgnoreCase(keywordText) && !"LOOP".equalsIgnoreCase(
            keywordText)
            && context.getBlockDepth() > 0) {
            context.decreaseBlockDepth();
        }
