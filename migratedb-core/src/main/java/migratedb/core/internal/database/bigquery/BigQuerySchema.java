...
public class BigQuerySchema extends BaseSchema<BigQueryDatabase, BigQueryTable> {
    ...
    protected boolean doExists() throws SQLException {
        ...
        try {
            return jdbcTemplate
                       .queryForInt(
                           "SELECT COUNT(table_name) FROM " + database.quote(name) + ".INFORMATION_SCHEMA.TABLES") >= 0;
    ...
    protected boolean doEmpty() throws SQLException {
        ...
        return doExists() &&
               (jdbcTemplate.queryForInt(
                   "SELECT COUNT(table_name) FROM " + database.quote(name) + ".INFORMATION_SCHEMA.TABLES")
                + jdbcTemplate.queryForInt(
                   "SELECT COUNT(routine_name) FROM " + database.quote(name) + ".INFORMATION_SCHEMA.ROUTINES")
                == 0);
    }
    ...
    private List<String> generateDropStatementsForRoutines(String objType) throws SQLException {
        List<String> objNames =
            jdbcTemplate.queryForStringList(
                ...
                "SELECT routine_name FROM " + database.quote(name) +
    ...
    protected BigQueryTable[] doAllTables() throws SQLException {
        ...
        List<String> tableNames =
            jdbcTemplate.queryForStringList(
                "SELECT table_name FROM " + database.quote(name) +
