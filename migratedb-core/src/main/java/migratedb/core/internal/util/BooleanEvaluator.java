...
public class BooleanEvaluator {
    ...
    public static boolean evaluateExpression(String expression) {
        ...
        if (expression.trim().equalsIgnoreCase("true")) {
            ...
            throw new MigrateDbException("Unable to parse expression: " + expression);
        }
    }
    ...
    private static String findInnermostBrackets(String expression) {
        ...
        for (int i = 0; i < expression.length(); i++) {
            ...
            if (expression.charAt(i) == ')') {
                if (openIndex == -1) {
                    throw new MigrateDbException("Unable to parse expression: " + expression + " - unexpected )");
                }
    ...
    private static boolean compareOperands(String operator, String expression) {
        ...
        if (operands.length != 2) {
            throw new MigrateDbException("Invalid boolean expression '" + expression + "'. ");
        }
