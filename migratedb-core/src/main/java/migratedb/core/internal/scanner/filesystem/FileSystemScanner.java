 ...
 */
package migratedb.core.internal.scanner.filesystem;
...
import migratedb.core.internal.resource.filesystem.FileSystemResource;
import migratedb.core.internal.sqlscript.SqlScriptMetadata;

public class FileSystemScanner {
    private static final Log LOG = Log.getLog(FileSystemScanner.class);
    private final Charset defaultEncoding;
    private final boolean detectEncoding;
    private final boolean throwOnMissingLocations;
    private final boolean stream = false;

    public FileSystemScanner(Charset encoding, boolean stream, boolean detectEncoding,
    ...
    public Collection<LoadableResource> scanForResources(Location location) {
        ...
        if (!dir.exists()) {
            if (throwOnMissingLocations) {
                throw new MigrateDbException("Failed to find filesystem location:" + path + ".");
            }
        ...
        if (!dir.canRead()) {
            if (throwOnMissingLocations) {
                throw new MigrateDbException("Failed to find filesystem location:" + path + " (not readable).");
            }
        ...
        if (!dir.isDirectory()) {
            if (throwOnMissingLocations) {
                throw new MigrateDbException("Failed to find filesystem location:" + path + " (not a directory).");
            }
        ...
        for (String resourceName : findResourceNamesFromFileSystem(path, new File(path))) {
            ...
            if (location.matchesPath(resourceName)) {
                ...
                if (new File(resourceName + ".conf").exists()) {
                    LoadableResource metadataResource = new FileSystemResource(location,
                                                                               resourceName + ".conf",
                                                                               defaultEncoding,
                                                                               false);
