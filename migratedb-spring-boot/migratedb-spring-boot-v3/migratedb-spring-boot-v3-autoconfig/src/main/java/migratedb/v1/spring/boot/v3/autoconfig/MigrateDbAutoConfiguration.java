...
import migratedb.v1.spring.boot.shared.DerivedDataSource;
import migratedb.v1.spring.boot.shared.MigrateDbInitializer;
import migratedb.v1.spring.boot.shared.MissingApplicationDataSourceException;
import migratedb.v1.spring.boot.shared.SpringIntegration;
...
public class MigrateDbAutoConfiguration {
    ...
    private void configureFromProperties(DefaultConfiguration configuration, @Nullable MigrateDbProperties props) {
        ...
        mapper.from(props.getCherryPick())
              .as(it -> it.stream().map(MigrationPattern::new).toList())
        ...
        mapper.from(props.getConnectRetriesInterval())
              ...
              .as(MigrateDbAutoConfiguration::clampToInt)
    ...
    private static Integer clampToInt(Long it) {
