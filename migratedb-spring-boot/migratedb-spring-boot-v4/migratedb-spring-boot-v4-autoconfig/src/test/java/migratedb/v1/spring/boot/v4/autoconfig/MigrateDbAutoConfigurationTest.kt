 ...
 * Copyright 2023-2026 The MigrateDB contributors.
 *
...
import migratedb.v1.spring.boot.shared.*
import migratedb.v1.spring.boot.v4.autoconfig.MigrateDbAutoConfigurationTest.DataSources.EMPTY_TABLE_INSIDE_MARKER_SCHEMA
import migratedb.v1.spring.boot.v4.autoconfig.MigrateDbAutoConfigurationTest.DataSources.MIGRATION_DS_MARKER_SCHEMA
import migratedb.v1.spring.boot.v4.autoconfig.MigrateDbAutoConfigurationTest.DataSources.evaluateUsingDataSource
...
internal class MigrateDbAutoConfigurationTest {
    ...
    fun `Can customize the MigrateDB execution which happens automagically on application startup`() {
        contextRunner.withUserConfiguration(
            MultipleUsersApplicationDataSourceConfiguration::class.java, FakeMigrateDbExecution::class.java
        ).run { context ->
            assertThat(context).singleBean<MigrateDb>()
            context.bean<FakeMigrateDbExecution>().assertCalled()
        }
    }
    ...
    fun `Spring beans implementing Callback are invoked in bean name order by default`() {
        contextRunner.withUserConfiguration(
            MultipleUsersApplicationDataSourceConfiguration::class.java,
            CallbackConfiguration::class.java
        ).run { context ->
            assertThat(context).singleBean<MigrateDb>().all { migrateDb ->
                val callback1 = context.getBean("callback1", RecordingCallback::class.java)
                val callback2 = context.getBean("callback2", RecordingCallback::class.java)
                migrateDb.configuration.callbacks.shouldContainExactly(callback1, callback2)

                val callback1Groups = callback1.invocations().groupBy { it.function }
                val callback2Groups = callback2.invocations().groupBy { it.function }

                callback1Groups.values.shouldNotBeEmpty()
                callback2Groups.values.shouldNotBeEmpty()

                callback1Groups.forEach { (function, callback1Invocations) ->
                    val callback2Invocations = callback2Groups[function]
                    callback2Invocations.shouldNotBeNull().apply {
                        shouldHaveSize(callback1Invocations.size)
                        forEachIndexed { index, callback2Invocation ->
                            callback1Invocations[index].wasBefore(callback2Invocation).shouldBeTrue()
                        }
                    }
                }
            }
    ...
    internal class CustomMigrateDbInitializerWithHighestPrecedenceOrder {
        ...
        fun customMigrateDbInitializer(migrateDb: MigrateDb): MigrateDbInitializer {
            val initializer = MigrateDbInitializer(migrateDb, DefaultMigrateDbExecution(false))
    ...
    internal class CustomMigrateDbMigrationInitializerWithJpaConfiguration {
        ...
        fun customMigrateDbMigrationInitializer(migrateDb: MigrateDb): MigrateDbInitializer {
            return MigrateDbInitializer(migrateDb, DefaultMigrateDbExecution(false))
        }
        ...
        fun entityManagerFactoryBean(dataSource: DataSource): LocalContainerEntityManagerFactoryBean {
            ...
            return org.springframework.boot.jpa.EntityManagerFactoryBuilder(
    ...
    @Component
    internal class FakeMigrateDbExecution : MigrateDbExecution {
    ...
    internal class CallbackConfiguration {
        ...
        @Bean
        fun callback1(): RecordingCallback {
            return RecordingCallback("callbackOne")
        }

        @Bean
        fun callback2(): RecordingCallback {
            return RecordingCallback("callbackTwo")
        }
    }
    ...
    /**
     * Asserts that MigrateDB uses a data source created by [DataSources.newMigrationDataSource] or by
     * [DataSources.newMigrationDataSourceUrl].
     */
